...
namespace AIM.Annotation
{
    ...
    internal class AimInstanceDictionary : IEnumerable<ISopInstance>
    {
        ...
        public void Add(string studyInstanceUID, string seriesInstanceUID, string sopInstanceUID, string instanceFileName)
        {
            ...
            if (this.ContainsStudy(studyInstanceUID))
            {
        ...
        public void Remove(string studyInstanceUID, string seriesInstanceUID, string sopInstanceUID)
        {
            ...
            if (this.ContainsInstance(studyInstanceUID, seriesInstanceUID, sopInstanceUID))
            {
                _studyDictionary[studyInstanceUID][seriesInstanceUID].Remove(sopInstanceUID);
                // Remove empty lookup dictionary
            ...
            // no-op if the instance is not present
        }

        public void Clear()
        {
            foreach (Dictionary<string, Dictionary<string, string>> studyInfo in _studyDictionary.Values)
            {
                foreach (Dictionary<string, string> seriesInfo in studyInfo.Values)
                {
                    seriesInfo.Clear();
                }
                studyInfo.Clear();
        ...
        public bool ContainsSeries(string studyInstanceUID, string seriesInstanceUID)
        {
            ...
            return this.ContainsStudy(studyInstanceUID) && _studyDictionary[studyInstanceUID].ContainsKey(seriesInstanceUID);
        }
        ...
        public bool ContainsInstance(string studyInstanceUID, string seriesInstanceUID, string sopInstanceUID)
        {
            ...
            return this.ContainsSeries(studyInstanceUID, seriesInstanceUID) && _studyDictionary[studyInstanceUID][seriesInstanceUID].ContainsKey(sopInstanceUID);
        }

        /// <summary>
        /// Total number of Annotation SOP instances
        /// </summary>
        /// <returns>Total number of Annotation SOP instances being tracked</returns>
        public int Count()
        {
            int count = 0;
            foreach(Dictionary<string, Dictionary<string, string>> studyInstanceInfo in _studyDictionary.Values)
            {
                Debug.Assert(studyInstanceInfo != null);
                foreach (Dictionary<string, string> seriesInfo in studyInstanceInfo.Values)
                {
        ...
        /// <summary>
        /// File Path Name of the SOP instance
        /// </summary>
        ...
        /// <returns>Annotation SOP Instance's file name or null if the instance is not found</returns>
        public string SopInstanceFileName(string studyInstanceUID, string seriesInstanceUID, string sopInstanceUID)
        {
            return this.ContainsInstance(studyInstanceUID, seriesInstanceUID, sopInstanceUID) ? _studyDictionary[studyInstanceUID][seriesInstanceUID][sopInstanceUID] : null;
        }
        ...
        public List<AimSopInstanceInformation> this[string studyInstanceUID]
        {
            get
            {
                ...
                if (this.ContainsStudy(studyInstanceUID))
                {
                    ...
                    foreach (string seriesInstanceUID in _studyDictionary[studyInstanceUID].Keys)
                    {
                        foreach (string sopInstanceUID in _studyDictionary[studyInstanceUID][seriesInstanceUID].Keys)
                            sopInstanceList.Add(
