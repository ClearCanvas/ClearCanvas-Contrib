...
namespace AIM.Annotation
{
	...
	//    [KeyboardAction("show", "imageviewer-keyboard/AIMAnnotationTool", "Show", KeyStroke = XKeys.A)]
	...
	public class AimAnnotationTool : ImageViewerTool
	{
		...
		private Dictionary<string, List<string>> _annotationFileList = new Dictionary<string, List<string>>();
		...
		//    {
		//        IImageSopProvider imageSop = e.SelectedTile.PresentationImage as IImageSopProvider;
		//        if (imageSop != null)
		//            this.InitializeAnnotationList(imageSop.ImageSop.StudyInstanceUID);
		//    }
		//}
		...
		{
			// TODO - monitor this for new annotations and update other (not the one on which it was drawn) images with the new annotation
		}

		private void OnInstanceDeleted(object sender, ItemEventArgs<DeletedInstanceInformation> e)
		{
			// TODO - if annotation is deleted, remove it from images:
			// TDB: for now, displayed annotations have no clue which file they belong to
		}

		private void OnLocalDataStoreCleared(object sender, EventArgs e)
		{
			// TODO - remove all saved annotations from all images:
			// TBD: for now, displayed annotations have no clue which file they belong to
		}
