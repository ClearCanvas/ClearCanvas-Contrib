...
{
    public partial class DropDownEntity : UserControl
    {
		public event EventHandler SelectedIndexChanged;

        public DropDownEntity(string label, IEnumerable<StandardCodeSequence> codeList, string helpText, bool addFirstDefault)
        {
            InitializeComponent();
            ...
            if (addFirstDefault)
                _ddlEntity.Items.Add(new ListItem(null));
            foreach (StandardCodeSequence codeSequence in codeList)
            {
                _ddlEntity.Items.Add(new ListItem(codeSequence));
            }

            if (_ddlEntity.Items.Count > 0)
                _ddlEntity.SelectedIndex = 0;

        	_ddlEntity.SelectedIndexChanged += OnSelectedIndexChanged;
        }

    	public StandardCodeSequence SelectedEntity
        {
            get
            {
                ListItem item = _ddlEntity.SelectedItem as ListItem;
            ...
            set
            {
				...
				{
					foreach (ListItem item in _ddlEntity.Items)
					{
						//if (item.Value == value)
						if (Equals(item.Value, value))
						{
							_ddlEntity.SelectedItem = item;
							return;
						}
					}
					Debug.Assert(false, "Trying to set unknown DropDownEntity value. Some data will be lost");
				}
            }
        }
    	...
    	{
			EventsHelper.Fire(SelectedIndexChanged, this, EventArgs.Empty);
    	}
    ...
    internal class ListItem
    {
		...
		public ListItem(StandardCodeSequence newValue)
        {
            _value = newValue;
        }
